...
# ========== WELCOME DATABASE FUNCTIONS ==========
from datetime import datetime, timedelta, timezone
from typing import Optional

_welcome_col = mongodb["welcome"]


async def is_on(chat_id: int) -> bool:
    doc = await _welcome_col.find_one({"_id": chat_id}, {"state": 1})
...
async def set_state(chat_id: int, flag: str) -> None:
    ...
    await _welcome_col.update_one({"_id": chat_id}, {"$set": {"state": flag}}, upsert=True)


async def bump(chat_id: int, time_window: int = 8) -> int:
    now = datetime.now(timezone.utc)
    doc = await _welcome_col.find_one({"_id": chat_id}, {"last_ts": 1, "join_cnt": 1})
...
async def auto_on(chat_id: int) -> bool:
    now = datetime.now(timezone.utc)
    doc = await _welcome_col.find_one({"_id": chat_id}, {"cool_until": 1})
